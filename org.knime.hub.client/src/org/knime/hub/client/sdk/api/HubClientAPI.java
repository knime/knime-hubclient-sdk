...
public class HubClientAPI implements AutoCloseable {
     ...
     * issue a PUT request to a path where groups are allowed (e.g. not at the space
     * level). If the group already exists, it will be returned. Groups are not
     * allowed to be created in a user directory, and can only be created within
     * spaces by non-admin users. To create a new space, issue a PUT request with a
     * request to a space path (i.e. in a user directory) with an optional request
     * body specifying space metadata. If the space already exists an error will be
     * returned. Space metadata properties not specified in the request body will be
     * set to their default values. If no request body is specified, default values
     * will be used.
     *
     * @param accountId         The ID of the account the repository item is
    ...
    /**
     * Deletes an item (workflow, data file, a whole workflow group) from the
     * repository.
     *
     * @param path              The absolute path to the repository item. It always starts
     *                          with a * and does not change even if the repository item is
     *                          renamed or moved. (required)
     * @param softDelete        Optional parameter that enables soft deletion of repository
     *                          items, which means that deleted items get moved to the
     *                          trash bin. (optional, default to false)
     * @param additionalHeaders Map of additional headers
     * @return {@link ApiResponse}
     *
     * @throws CouldNotAuthorizeException
     ...
     *                          with (required)
     * @param subPath           The "/" delimited path to the resource below the
     *                          account root level (required)
     * @param softDelete        Optional parameter that enables soft deletion of repository
     *                          items, which means that deleted items get moved to the
     *                          trash bin. (optional, default to false)
     * @param additionalHeaders Map of additional headers
     ...
     * repository.
     *
     * @param id                The items unique ID. It always starts with a * and
     *                          does not change even if the repository item is renamed or
     *                          moved. (required)
     * @param softDelete        Optional parameter that enables soft deletion of repository
     *                          items, which means that deleted items get moved to the
     *                          trash bin. (optional, default to false)
    ...
    /**
     * Downloads the given repository item. The returned content depends on the item type. If its a workflow
     * then a ZIP archive is returned. If its a data file then the file will be
     ...
     *                              <li>The special value "current-state", which points to the latest
     *                          overall state of the item, i.e. including unversioned changes.</li>
     *                          </ul>
     *                          If used together with query parameter
     *                          "spaceVersion" the two parameters need to refer
     *                          to the same version. (optional, default to
     *                          current-state)
     ...
     *                          only use the "version" parameter instead.
     ...
     *                          with a * and does not change even if the repository
     *                          item is renamed or moved. May also be a concatenation
     *                          of path followed by the "~" character and the
     *                          ID without the leading "*" character. This
     *                          occurs when the request originates from an older AP
     ...
     *                          (optional, default to -1)
     * @param responseType      The type of the response body (required).
     * @param contentHandler    The content handler to out source the given input stream
     *                          (required).
     * @param additionalHeaders Map of additional headers
     * @return {@link ApiResponse}
     *
     ...
     * @param deep              Optional query parameter which enables deep listing of
     *                          all children. (optional)
     * @param spaceDetails      Optional query parameter which when true provides
     *                          additional information about a space (i.e. kudos and
     *                          stats) (optional)
     * @param contribSpaces     An optional query parameter which determines if the
     *                          spaces a user can contribute to are included when
     *                          requesting a users directory, and the format these
     *                          spaces should be written in. This parameter is ignored
     *                          if another other than a users directory is
     *                          requested. (optional, default to none)
     * @param version           Optional query parameter which specifies the version of
     *                          the item to retrieve.
     *                          <ul>
     *                              <li>The version number of an existing item version.
    ...
    /**
     * Returns the repository item at the given path or with the given ID in the
     * server repository.
     *
     * @param id                The items unique ID. It always starts with a * and
     *                          does not change even if the repository item is renamed
     *                          or moved. May also be a concatenation of path followed
     *                          by the "~"; character and the ID without the
     *                          leading "*" character. This occurs when the
     *                          request originates from an older AP that cannot handle
     *                          the new URI format which adds the ID to the end of the
     *                          path. (required)
     * @param details           Specifies whether details should be shown and in what
     *                          form (full details, aggregated, ...). (optional)
     * @param deep              Optional query parameter which enables deep listing of
     ...
     *                              <li>The special value "current-state", which points to the latest
     *                          overall state of the item, i.e. including unversioned changes.</li>
     *                          </ul>If used together with query parameter
     *                          "spaceVersion" the two parameters need to refer
     *                          to the same version. (optional, default to
     *                          current-state)
     * @param spaceVersion      Legacy: Optional query parameter which indicates the
    ...
    /**
     * Prepares the upload of a repository item as a direct child of the item with
     * the given ID, and with the given content type. This will create any necessary
     * nested Workflow Groups lazily. Additionally, this endpoint will lock all
     * requested paths until the upload completes.
     *
     * @param parentId          The ID of the parent item the artifact will be uploaded
     *                          into. (required)
    ...
    public ApiResponse<UploadStatus> pollUploadStatus(final String uploadId,
        ...
        final var requestPath = IPath.forPosix(UPLOAD_API_PATH)
                .append(uploadId)
                .append(PATH_PIECE_UPLOAD_STATUS);
